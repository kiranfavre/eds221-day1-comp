...
We use read_csv to help account for the different types of data in our set
...
- Second: What is that warning? `problems()` - line 6424
   We are having one or more parsing issues: it is probably because of how R   thinks the data should be stored
...
```{r}
toolik_weather <- toolik_weather %>% clean_names() # What does this do? Say it in words. It takes something messy (like column names in this example) and converts into a much more doer friendly name
...
# using ggsave to save our figure to the figs folder and under the name toolik_temp_plot.
#ggsave is set to default to plotting to the last plot made in the code
```
